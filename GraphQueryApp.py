...
def set_natural_query()->None:
    ...
    input_text = st.selectbox("Select a question", ["China+South China Sea"], index=0)

    selected_relation = st.selectbox("Select a theme", list(event_code_dict.keys()), index=285)
...
question_templates={
    "China+South China Sea":("CHINA", "180", []),
}
...
if st.button("query"):
    query_embedding = get_doc_embedding(use_local=False)
    output_table = graph_doc_matching(query_embedding, use_local=False, use_cluster=False)
